...
- [x] Tang9k result-window smoke path: `hal::serial` now defines `ResultRead32/ResultValue32`, golden vectors cover the wire bytes, `hal-ffi::serial_backend` exposes `tang9k_probe --result-smoke` and `--result-window-smoke`, and the responder RTL records a deterministic 4-word MatMul summary window. Real-board SRAM reflash via `USB Debugger A` passed on `COM3 @ 115200`, returning `0x00003005, 0x9e3749bc, 0x3c6ec377, 0xdaa65d2e`.
